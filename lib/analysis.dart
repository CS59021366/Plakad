...
//          break;
  ...
  Future recognizeImage1(File image) async {
    var recognitions = await Tflite.runModelOnImage(
      path: image.path,
      numResults: 6,
      threshold: 0.05,
  ...
  Widget build(BuildContext context) {
    ...
    if (_model == deeplab && _recognitions != null) {
      ...
      stackChildren.add(Positioned(
        ...
        left: 0.0,
        width: size.width,
        child: _image == null ? Text('ยังไม่มีรูปภาพที่จะวิเคราะห์',style: TextStyle(color: Colors.black,fontSize: 20.0)) : Image.file(_image),
      ));
    }
...
//    }
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        title: const Text('วิเคราะห์โดยระแบบ'),
        actions: <Widget>[
